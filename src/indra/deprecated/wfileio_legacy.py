...
# TODOs:
# 1. Implement something to convert GHI to DNI and DHI.
...
keywords = dict(
    tmy=("nrel", "iwec", "ishrae", "cwec", "igdg", "tmy3", "meteonorm"),
    amy=("ncdc", "nsrdb", "nrel_indiasolar", "ms", "WY2", "nasa_saudi"),
)
...
def get_weather(stcode, fpath, logger=None):
    ...
    if not os.path.isfile(fpath):
        logger.info(
            "I cannot find file %s. Returning empty dataframe.",
            fpath,
        )
    ...
    if file_type == "pickle":
        try:
            ...
            logger.info(
                "You asked me to read a pickle but I could not. Trying all other formats."
            )
        ...
        try:
            wdata, locdata, header, columns = read_espr(fpath, logger=logger)
            # Remove leap day.
            wdata = petite.remove_leap_day(wdata)
        except Exception as err:
            logger.info("Error: %s", err)
        ...
        try:
            ...
            locdata = dict(
                loc=stcode, lat="00", long="00", tz="00", alt="00", wmo="000000"
            )
            header = (
                "# Unknown incoming file format "
                + "(not epw or espr)\r\n"
                + "# Dummy location data: "
        ...
        try:
            wdata, locdata, header = read_fin4(fpath, logger=logger)
        except Exception as err:
            logger.info("Error: %s", err)
            wdata = None
            header = None
    ...
    if wdata is None:
        logger.info(
            "I could not read the file you gave me with the format you specified. Trying all readers."
        )
        ...
        date_index = pd.date_range(
            start="{:d}-01-01 00:00:00".format(int(wdata["year"].iloc[0])),
            end="{:d}-12-31 23:00:00".format(int(wdata["year"].iloc[0])),
            freq="1h",
        )
        wdata.index = date_index[~((date_index.day == 29) & (date_index.month == 2))]

        logger.info("get_weather success")
        return wdata, locdata, header


def read_fin4(fpath, logger=None):
    logger = get_logger(logger)
    ...
    if wdata.shape[0] == temp_index.shape[0]:
        ...
        wdata.index = temp_index[~((temp_index.day == 29) & (temp_index.month == 2))]
    ...
    wdata["rh"] = pd.Series(
        petite.calc_rh(wdata["tdb"], wdata["tdp"]), index=wdata.index
    )

    logger.info("read_fin4 success")
    return wdata, locdata, header
...
def day_of_year(month, day, logger=None):
    logger = get_logger(logger)
    ...
    logger.info("day_of_year success")
    return doy
...
def day_of_month(day, logger=None):
    logger = get_logger(logger)
    ...
    logger.info("day_of_month success")
    return month, dom
...
def read_epw(fpath, epw_colnames=epw_colnames, logger=None):
    logger = get_logger(logger)
    ...
    dates = pd.date_range(
        start="{}-01-01 00:00:00".format(wdata["year"].unique()[0]),
    ...
    if len(wdata.columns) == 35:
        ...
        wdata = wdata.drop(["unknownvar1", "unknownvar2", "unknownvar3"], axis=1)
    ...
    if wdata.empty:
        logger.info(
            "Could not locate a file with given station name. Returning empty table."
        )
...
def read_espr(fpath, logger=None):
    logger = get_logger(logger)
    ...
    if content[0].strip() == "*WEATHER 2":
        ...
        logger.info("Error: Format of ESP-r weather file not recognised")
        clmdata = None
        locdata = None
        header = None
        logger.info("read_espr success")
    ...
    daylines = [[idx, line] for [idx, line] in enumerate(body) if "day" in line]
    ...
    dataout = np.concatenate(
        (np.reshape(np.repeat(int(year), 8760), [-1, 1]), dataout), axis=1
    )

    clmdata = pd.DataFrame(
        data=dataout,
        index=dates,
        columns=[
            "year",
...
def give_weather(
    ...
    if path_file_out == ".":
        ...
        filepath = os.path.join(
            path_file_out, "wf_out_{0}_{1}".format(np.random.randint(0, 99, 1), year)
        )
    else:
        # Files need to be renamed so strip out the extension.
        filepath = (
            path_file_out.replace(".a", "").replace(".epw", "").replace(".csv", "")
        )
    ...
    if file_type == "espr":
        ...
        esp_master, locdata, header, esp_columns = read_espr(masterfile, logger=logger)

        # Replace the year in the header.
        yline = [line for line in header if "year" in line]
        yval = yline[0].split(",")
        yline[0] = yline[0].replace(yval[0], str(year))
        header = [yline[0] if "year" in line else line for line in header]
        ...
        esp_master = esp_master.drop(
            ["year", "month", "day", "hour", "ghi", "tdp"], axis=1
        )
        ...
        epw_master, locdata, header = read_epw(masterfile, logger=logger)
        # Cut out the last new-line character since numpy savetxt
        # puts in a newline character after the header anyway.
        header[-1] = header[-1][:-1]
        ...
        _, _, header = read_fin4(masterfile, logger=logger)
        ...
        fin_fmt = ["%4d", "%2d", "%2d", "%2d"] + (
            (np.repeat("%6.1f", len(df.columns) - (4))).tolist()
        )
    ...
    if success:
        ...
        logger.info("Some error prevented file from being written.")
